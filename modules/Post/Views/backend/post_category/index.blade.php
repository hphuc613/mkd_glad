    ...
    <div id="post-module">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h4 class="title">{{ trans("Post Category") }}</h4>
            </div>
            <div class="col-md-7 align-self-center text-right">
                <div class="d-flex justify-content-end align-items-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">{{ trans("Home") }}</a></li>
                        <li class="breadcrumb-item active">{{ trans("Post Category") }}</li>
                    </ol>
                </div>
            </div>
        </div>
        <div class="mb-3 d-flex justify-content-end group-btn">
            <a href="{{ route('get.post_category.create') }}" class="btn btn-primary"
               data-toggle="modal" data-target="#form-modal" data-title="{{ trans("Create Post Category") }}">
                        ...
                        @foreach($data as $item)
                            <tr>
                                ...
                                <td class="link-action">
                                    <a href="{{ route('get.post_category.update', $item->id) }}" class="btn btn-primary mr-2"
                                       data-toggle="modal" data-target="#form-modal"
                                       data-title="{{ trans("Update Post Category") }}">
                                        <i class="fa fa-pencil"></i></a>
                                    <a href="{{ route('get.post_category.delete', $item->id) }}"
