            ...
            <div class="col-md-7 align-self-center text-right">
                <div class="d-flex justify-content-end align-items-center">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{ trans("Shipping") }}</li>
                    </ol>
                </div>
            </div>
        </div>
        <div class="mb-3 d-flex justify-content-end group-btn">
            <a href="{{ route("get.shipping.create") }}" class="btn btn-primary"
               data-toggle="modal" data-target="#form-modal" data-title="{{ trans("Create Shipping") }}">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="text-input">{{ trans("Shipping name") }}</label>
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach($data as $item)
                            <tr>
                                ...
                                <td>{{ trans($item->value) }}</td>
                                <td>{{ \Modules\Base\Models\Status::getStatus($item->status) ?? null }}</td>
                                <td>{{ \Carbon\Carbon::parse($item->created_at)->format('d/m/Y H:i:s')}}</td>
                                <td>{{ \Carbon\Carbon::parse($item->updated_at)->format('d/m/Y H:i:s')}}</td>
                                <td class="link-action">
                                    <a href="{{ route('get.shipping.update', $item->id) }}" class="btn btn-primary"
                                       data-toggle="modal" data-target="#form-modal"
                                       data-title="{{ trans("Update Shipping") }}">
                                        <i class="fas fa-pencil-alt"></i></a>
                                    <a href="{{ route('get.shipping.delete', $item->id) }}"
