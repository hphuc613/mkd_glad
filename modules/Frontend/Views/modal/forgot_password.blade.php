            ...
            <div class="h-100">
                ...
                <div class="form-forgot">
                    <form action="{{ route('post.home.forgotPassword') }}" method="post">
                        ...
                        <h3>{{trans('忘記密碼')}}</h3>
                        <hr>
                        <div class="title-noti">{{trans('我們將會發送電郵給你重設密碼')}}</div>
                        <div class="form-group">
                            <label class="label-input-login" for="email-forgot">
                                {{trans('電郵地址')}} <span> *</span>
                            </label>
                            <input type="text" id="email-forgot" name="email" placeholder="電郵地址"
                                   class="form-control input-login">
                        </div>
                        <div class="row-send">
                            <button type="submit" class="btn btn-main px-5 py-2 rounded-0">{{trans('提交')}}</button>
                            <div class="title-or">{{trans('或')}}</div>
                            <a href="javascript:"  data-bs-dismiss="modal" class="text-decoration-none title-cancel">{{trans('取消')}}</a>
                        </div>
                        <div class="divider one-line">{{trans('或')}}</div>
                        <div class="title-forgot mb-1">{{trans('新客戶')}}</div>
                        <div class="title-noti mb-lg-3 mb-5">{{trans('申請賬戶後付款更快捷、查看訂單及其他資料')}}</div>
